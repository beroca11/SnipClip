...
export async function runMigrations() {
  ...
  try {
    ...
    // Run data migration for PostgreSQL only
    if (!isSQLite) {
      await migrateJsonDataToPostgreSQL();
    }
...
async function migrateJsonDataToPostgreSQL() {
  ...
  if (!fs.existsSync(dataDir)) {
    console.log("No data directory found, skipping JSON migration");
    return;
  }
  ...
  try {
    ...
    if (migrationCheck?.rows?.[0]?.count > 0) {
      console.log("Database already contains data, skipping JSON migration");
      return;
    }
